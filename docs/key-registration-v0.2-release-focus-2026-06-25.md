...
- [ ] 物理オンオフキー
  - 既存の `physicsToggles` pass-through と、MMD_modoki 上の編集対象を分けて整理する。
  - 手動登録時は VMD loader と同じ既定値 `1` を基本にし、明示的な ON/OFF 編集を追加する。
  - seek / playback / project round-trip で runtime と表示が一致することを確認する。
...
4. 物理オンオフキーを、既存 `physicsToggles` の表示 / 編集 / 保存へつなぐ。
