...
- `ui-controller.ts` は 2026-04 時点の 7,800 行超から 4,603 行まで減った。
- 行数ベースでは約 3 分の 1 弱を外へ逃がせた。
- 安全に切り出しやすい toolbar / panel / export / accessory / layout はかなり進んだ。
- 右パネル PostFX の実処理はおおむね controller 側へ移した。
- まだ残っている大きな塊は、右パネル HTML composition、camera controls、model info、timeline / keyframe 編集。
