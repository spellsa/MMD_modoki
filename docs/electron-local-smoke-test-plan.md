...
- main window 作成後、`ready-to-show` または `did-finish-load` を成功条件にする。
...
if (isSmokeMode) {
    ...
    mainWindow.once("ready-to-show", finishOk);
...
- 成功条件は厳しくしすぎない。最初は「ウィンドウが出る / renderer が即死しない」でよい。
- Canvas 表示や GPU 初期化成功を必須にしない。

### 2. Node 製 smoke-launch script

Windows / macOS / Linux で環境変数指定を揃えるため、npm script に直接 `MMD_MODOKI_SMOKE=1` を書かず、Node script から `electron-forge start` を spawn する。
...
- `electron-forge start` を子プロセスとして起動する。
...
const timeoutMs = Number(process.env.MMD_MODOKI_SMOKE_TIMEOUT_MS ?? 30000);
...
- `npm start` を経由すると script 名の変更に強い。
- ただし `npm start -- --no-sandbox` が Electron Forge 側へ期待通り渡るかは実機で確認する。
