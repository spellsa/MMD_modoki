...
- [ ] MMM / 動画投稿サイトのようなシークバー導入を検討する
  - まずは独立したバーを増やさず、既存タイムライン目盛り / 波形行のクリック・ドラッグ seek を強化する案を優先候補にする
  - 余裕があれば、ビューポート下バー上端に薄い progress / seek line を重ねる案も検討する
  - 現状の v0.2 UI は配置余白が少ないため、再生欄やビューポート下バーへ常設の大きなシークバーを追加する案は慎重に扱う
