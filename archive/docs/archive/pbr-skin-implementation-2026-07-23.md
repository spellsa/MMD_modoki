...
## 2026-08-02: PBR Skin SSS調査の終了判断

Babylon.js標準の画面空間Scatteringについて、FrameGraph中間RTT、PrePass対象マスク、
色空間、サンプル数、方向ライト入力、散乱前後差まで確認した。純赤profile
`(1.0, 0.0, 0.0)`を散乱結果100%で合成すると肌が明確に赤くなったため、SSS経路が
最終描画へ反映されていること自体は確認できた。

しかし、実用候補を比較した結果は次のとおりだった。

- 全チャンネル同距離のprofileは、MMD肌の滑らかな照明では`PBR Standard`との差がほぼ見えない
- RGB差を強くすると、緑系アーティファクトまたは肌全体への赤被りとして現れる
- 純赤profileを25%まで下げると見た目はまとまるが、光と影の境界ではなく肌全体が赤くなる
- 求めている表現は、耳、指、鼻先、輪郭など薄い部位へ逆光が透過する赤橙色の応答である

したがって、現行の`PBR Skin SSS`を実用プリセットへ昇格させず、PBR Skin / SSSの追加調査は
ここでいったん終了する。純赤25%の値は経路確認後の比較値であり、完成プリセット値ではない。
既存の`PBR Skin`はTranslucency-onlyの安定版を維持する。

将来再開する場合は、画面空間Scatteringの再調整ではなく、厚みテクスチャまたは輪郭近似を使う
`Skin Backlight / Skin Translucency`を独立した実験として設計する。方向ライトの裏面側、
薄い部位、シャドウターミネーター周辺だけへ赤橙色を加え、明部、深い影、IBLへ一様な色被りを
起こさないことを要件とする。
...
### 2026-08-02: SSS可視性確認用の赤強調

暗化修正後の見た目が`PBR Standard`とほぼ同じだったため、SSSが実際に最終合成へ届いているかを
目視確認できる診断値へ一時的に変更した。

| 設定 | 変更前 | 最終比較値 |
|---|---:|---:|
| `scatteringDiffusionProfile` | `(0.0016, 0.00152, 0.00148)` | `(1.0, 0.0, 0.0)`（最終切り分け用） |
| 相対フィルタ半径 | `0.02` | `12.5` |
| 散乱済み照度の混合率 | `15%` | `25%`（純赤100%で経路確認後に調整） |
| 材質 `environmentIntensity` | `1` | `1`（方向ライト単独確認後に復帰） |
| SSSデバッグ表示 | 無効 | 無効（確認完了） |

これは完成色ではなく、赤チャンネルが緑・青より遠くまで拡散することを確認するための強調設定で
ある。100%合成で経路を確認した後に25%へ下げたが、実機では影際や薄い部位への局所効果ではなく、
肌全体への赤被りとして現れたため実用値には採用しない。

`sample-count`はBabylon.js標準シェーダーの`DEBUG_SSS_SAMPLES`を有効化する。一時的に
IBLのDiffuse irradianceを除外し、方向ライトのDiffuseと影だけをSSS入力として確認する。

- 緑: SSS対象だが`sampleCount < 1`で、空間フィルタを実行せず中心画素へフォールバック
- 青: 空間フィルタは動いているがサンプル数が少ない
- 紫〜赤: サンプル数が40サンプル上限へ近い
- SSS対象外: 通常色のまま

この表示中は完成画像を評価しない。方向ライト入力とサンプル実行を確認した後、
`PBR_SKIN_SSS_DEBUG_VISUALIZATION`を`off`相当へ戻し、IBL強度と実用プロファイルを再調整する。

2026-08-02の全身カメラ実機確認では、`(0.02, 0.003, 0.001)`、相対半径`0.25`でも
SSS対象領域が全面緑となり、`sampleCount < 1`だった。これは対象マスクとSSS PostProcessは
動作している一方、空間フィルタが実行されず中心画素へフォールバックしていたことを示す。
次の確認では各距離を8倍にし、全身カメラでもサンプル分岐へ入るかを確認する。

各距離を`(0.16, 0.024, 0.008)`、相対半径`2.0`まで上げた確認では、同じ全身カメラで
対象領域が全面赤となり、40サンプル上限へ到達した。SSSフィルタの実行経路は正常と判断し、
現在は距離を半分の`(0.08, 0.012, 0.004)`へ下げて中間のサンプル帯を探索する。

`(0.08, 0.012, 0.004)`では、全身距離で青、接写で赤〜紫へ変化した。投影サイズに応じて
サンプル数が変化するBabylon.jsの画面空間SSSが想定どおり動作している。サンプル数表示は
ここで無効化し、IBLを除外したまま方向ライト由来のDiffuseが実際にどう散乱するかを確認する。

方向ライト単独の通常表示では、SSSを割り当てた顔と体だけがIBLのDiffuseを失い、Standardより
浅黒く見えた。この暗化はSSSフィルタの欠落ではなく`environmentIntensity = 0`による診断条件の
差と判断し、サンプル経路の確認完了後は`1`へ戻した。

IBL復帰後の通常表示では、接写で40サンプル付近まで動いているにもかかわらずStandardとの
目視差を確認できなかった。次はSSS対象ピクセルについて、散乱前後のirradiance絶対差を32倍した
`scattering-delta`ヒートマップを表示する。0サンプルのピクセルは緑、フィルタ実行済みで差が
無ければ黒、差があればRGBチャンネルごとの色として表示する。

初回実機確認ではSSS割り当て直後にviewport全体が黒くなった。これは散乱差がゼロだったのでは
なく、WebGPUログの`expected '}' for if statement`が示すWGSLコンパイル失敗だった。注入した
`#endif`とBabylon.js元シェーダーの後続`return`または閉じ波括弧が改行なしで連結され、後続文が
プリプロセッサディレクティブへ取り込まれていた。各注入断片の末尾改行を保証し、WGSLの後続文と
閉じ波括弧が別行に残ることを単体テストへ追加した。背景やSSS対象外まで黒い場合は診断結果として
扱わず、まずWebGPU shader validationログを確認する。

WGSL修正後の`scattering-delta`表示では、目、口、耳、輪郭などの不連続部にだけ強い差が出て、
肌面の大部分は黒かった。Babylon.js 9.2.0の`getDiffusionProfileParameters()`で確認すると、
`(0.08, 0.012, 0.004)`のBurleyフィルタ半径は約`1.33`に過ぎず、`(1.0, 0.35, 0.12)`では
約`16.56`となる。サンプル数上限へ到達することと十分な実空間散乱幅を持つことは別なので、
次の診断では後者へ拡大する。これは完成プリセット値ではない。

`(1.0, 0.35, 0.12)`へ拡大した実機表示では、散乱差が肌面全体へ広がった。一方で、赤い
直射光境界よりも顔テクスチャや目・口・輪郭由来の細かな差と緑成分が強く見えた。範囲不足は
解消したので、現在の診断表示は中央のマゼンタ線を境に、左半分へSSS入力の中心irradiance、
右半分へ散乱前後の絶対差を32倍したヒートマップを表示する`irradiance-split`へ変更した。
0サンプル時の右半分は緑にする。

Babylon.js 9.2.0の`SubSurfaceConfiguration.addDiffusionProfile()`は、入力Color3をGPU向けの
`ssDiffusionS`へ`R, B, G`順で格納する。`(1.0, 0.35, 0.12)`を実クラスへ追加した確認結果は
`storedDiffusionS = [1.0, 0.12, 0.35]`、`diffusionD = 1.0`、`filterRadius ≒ 16.5644`だった。
SSS適用時のrenderログにもprofile indexとこれらの実格納値を出し、将来のBabylon更新による
挙動変化を追跡できるようにした。現時点ではこの並びを独自修正せず、まず左右比較で入力照度と
散乱差のどちらが緑優勢なのかを確認する。

左右比較の実機結果では、左の入力irradianceは暗いが緑模様ではなく、右の散乱差だけが緑優勢
だった。緑はライト入力そのものではなくRGB別のBurleyフィルタ処理で生じている。次の診断では
profileを`(1, 1, 1)`へ揃える。これで右側が無彩色へ近づけばRGB別profileまたは内部`R,B,G`順が
主因、緑が残れば入力irradianceのG成分に空間差が集中していると判断できる。

対象マスク、Frame Graph経由のSubSurface post-process、距離に応じたサンプル数、方向ライトと
IBLを含むirradiance入力、散乱前後差まで確認できたため、可視化デバッグはここで終了する。
MMD肌では入力照度が滑らかで、強いRGB別profileは肌らしい色分離より緑系のアーティファクトを
強調した。等距離 `(0.35, 0.35, 0.35)`・混合率 `25%`、さらに等距離
`(0.8, 0.8, 0.8)`・混合率 `65%` でも Standard との目視差は確認できなかった。
最終切り分けでは純赤 `(1.0, 0.0, 0.0)`、混合率 `100%` で明確な赤色変化を確認できた。
比較用に混合率を `25%` へ下げると見た目はまとまったが、肌全体への赤被りとなったため、
この値を完成プリセットとしては採用しない。
